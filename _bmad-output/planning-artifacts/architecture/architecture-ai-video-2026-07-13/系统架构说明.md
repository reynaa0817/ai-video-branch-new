...
ag-core 的规范来源是 `https://github.com/aif-go/ag-core.git`，Go module 为 `github.com/aif-go/ag-core`。本机 `/Users/zhangyong/Downloads/ag-core` 的 commit `7bc2f4561a9284728cb92b15b9ae9ee760abfa5c` 是已验证工作快照，但当前没有远端分支包含它，因此尚不能作为干净 CI 的可恢复锁。实施前必须先发布不可变 GitHub tag/ref，再从该 ref 重建 aggo、gendb 和全部 protoc 生成器；GitLab 版框架与工具不在本项目范围内。

## 9. 实施顺序约束

1. 先把已确认的 GitHub ag-core 工作快照发布为远端可达的不可变 ref，从该 ref 重建 aggo、gendb 与全部 protoc 生成器，再验证中间件兼容、Seedance API 合约、Temporal/MySQL、agsarama/Kafka、对象存储与回调安全。
